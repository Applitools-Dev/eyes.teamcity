 ...
 *
 * **Example.**
 * Adds an [Xcode](https://www.jetbrains.com/help/teamcity/?Xcode+Project) build step with a scheme-based build.
 * ```
 * xcode {
 *     name = "My Xcode step"
 ...
 * **Example.**
 * Adds an [Xcode](https://www.jetbrains.com/help/teamcity/?Xcode+Project) build step with a scheme-based build
 * with custom [working directory](https://www.jetbrains.com/help/teamcity/?Build+Working+Directory),
 * with custom Xcode path, output directory, build actions and additional command line parameters.
 * ```
 * xcode {
 *     name = "My Xcode step"
 *     workingDir = "project"
 ...
 *
 * **Example.**
 * Adds an [Xcode](https://www.jetbrains.com/help/teamcity/?Xcode+Project) build step with a target-based build.
 * ```
 * xcode {
 *     name = "My Xcode step"
 *     projectPath = "SampleProject.xcodeproj"
...
open class XcodeStep : BuildStep {
    constructor(init: XcodeStep.() -> Unit = {}, base: XcodeStep? = null): super(base = base as BuildStep?) {
        type = "Xcode"
        init()
    }
    ...
    sealed class BuildType(value: String? = null): CompoundParam(value) {
        ...
        class SchemeBased() : BuildType("4") {
            ...
            sealed class OutputDirectory(value: String? = null): CompoundParam(value) {
 ...
 * @see XcodeStep
 */
fun BuildSteps.xcode(base: XcodeStep? = null, init: XcodeStep.() -> Unit = {}) {
    step(XcodeStep(init, base))
}
