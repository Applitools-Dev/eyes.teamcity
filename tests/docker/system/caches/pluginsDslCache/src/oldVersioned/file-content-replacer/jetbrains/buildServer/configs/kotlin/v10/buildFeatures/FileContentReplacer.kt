 ...
 *
 * **Example.**
 * Finds all the 'Properties/AssemblyInfo.cs' files on the agent under the build checkout directory and performs replacement there according to the provided regular explression.
 * All the strings matched by the regexp will be replaced with the current build number.
...
open class FileContentReplacer : BuildFeature {
    constructor(init: FileContentReplacer.() -> Unit = {}, base: FileContentReplacer? = null): super(base = base as BuildFeature?) {
 ...
 */
fun BuildFeatures.replaceContent(base: FileContentReplacer? = null, init: FileContentReplacer.() -> Unit = {}) {
    feature(FileContentReplacer(init, base))
}
