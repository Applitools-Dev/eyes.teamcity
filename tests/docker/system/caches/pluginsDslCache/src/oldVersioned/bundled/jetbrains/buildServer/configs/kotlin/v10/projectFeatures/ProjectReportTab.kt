 ...
 *
 * **Example.**
 * Creates a tab on the project level which will show content produced by the latest build with tag "release" in the build configuration with id "MyBuild".
 * The tab title is "Release Documentation".
 * ```
 * projectReportTab {
...
open class ProjectReportTab : ProjectFeature {
    constructor(init: ProjectReportTab.() -> Unit = {}, base: ProjectReportTab? = null): super(base = base as ProjectFeature?) {
        type = "ReportTab"
 ...
 * @see ProjectReportTab
 */
fun ProjectFeatures.projectReportTab(base: ProjectReportTab? = null, init: ProjectReportTab.() -> Unit = {}) {
    feature(ProjectReportTab(init, base))
}
