 ...
 *
 * **Example.**
 * Trigger a build every day at 10AM even if there are no pending changes.
 * ```
 * schedule {
 ...
 *
 * **Example.**
 * Check for pending changes twice per hour in branches with name starting with 'feature/' and trigger builds there if pending changes found.
 ...
 *
 * **Example.**
 * Trigger a build every day at 10AM but only if there is a freshly finished build in a build configuration with id "MyApp".
 * See also [build changes](https://www.jetbrains.com/help/teamcity/configuring-schedule-triggers.html#Build+Changes).
 * ```
 * schedule {
 *   schedulingPolicy = daily {
 *     hour = 10
 ...
 *
 * **Example.**
 * Trigger a build at 10AM every day with an additional parameter "release" set to "true".
 * See also [triggered build customization](https://www.jetbrains.com/help/teamcity/configuring-schedule-triggers.html#Triggered+Build+Customization).
 * ```
 * schedule {
 *   schedulingPolicy = daily {
 *     hour = 10
