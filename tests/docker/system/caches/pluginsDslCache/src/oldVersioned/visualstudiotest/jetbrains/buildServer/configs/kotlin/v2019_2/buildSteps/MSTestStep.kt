...
/**
 * Deprecated, was previously used to set up a [build step for running MSTest tests](https://www.jetbrains.com/help/teamcity/visual-studio-tests.html).
 * Use the [.NET VSTest runner](https://www.jetbrains.com/help/teamcity/kotlin-dsl-documentation/buildSteps/dotnet-vs-test-step/index.html) instead.
 * See [this link](https://www.jetbrains.com/help/teamcity/net.html#Migrating+from+Visual+Studio+Tests+Runner) for more information on migrating to the .NET VSTest runner.
 *
 *
 * @see mstest
 */
@Deprecated("Deprecated, was previously used to set up a build step for running MSTest tests.", replaceWith = ReplaceWith("DotnetVsTestStep"))
open class MSTestStep() : BuildStep() {
    ...
    constructor(init: MSTestStep.() -> Unit): this() {
        init()
    }
    ...
    override fun validate(consumer: ErrorConsumer) {
        ...
        if (mstestPath == null && !hasParam("vstest_runner_path")) {
            consumer.consumePropertyError("mstestPath", "mandatory 'mstestPath' property is not specified")
        }
