...
open class GitVcsRoot() : VcsRoot() {
    ...
    sealed class AuthMethod(value: String? = null): CompoundParam<AuthMethod>(value) {
        ...
        class Token() : AuthMethod("ACCESS_TOKEN") {

            /**
             * Username to use with the token
             */
            var userName by stringParameter("username")

            /**
             * Compound id of a token in the token storage
             */
            var tokenId by stringParameter()

            override fun validate(consumer: ErrorConsumer) {
                if (tokenId == null && !hasParam("tokenId")) {
                    consumer.consumePropertyError("authMethod.tokenId", "mandatory 'authMethod.tokenId' property is not specified")
                }
            }
        }
    }
     ...
     */
    fun token(init: AuthMethod.Token.() -> Unit = {}) : AuthMethod.Token {
        val result = AuthMethod.Token()
        result.init()
        return result
    }
