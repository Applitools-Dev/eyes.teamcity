...
open class RakeStep : BuildStep {
    constructor(init: RakeStep.() -> Unit = {}, base: RakeStep? = null): super(base = base as BuildStep?) {
        type = "rake-runner"
        init()
    }
    ...
    sealed class Rakefile(value: String? = null): CompoundParam(value) {
        class File() : Rakefile("") {
    ...
    sealed class RubyInterpreterMode(value: String? = null): CompoundParam(value) {
 ...
 * @see RakeStep
 */
fun BuildSteps.rake(base: RakeStep? = null, init: RakeStep.() -> Unit = {}) {
    step(RakeStep(init, base))
}
